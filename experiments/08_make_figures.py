...
def make_feature_importance_figure(btc):
    ...
    ax.text(0.99, -0.16,
            ...
            transform=ax.transAxes, ha="right", va="top", fontsize=9.5, color=INK_SECONDARY)
...
def make_auc_by_horizon_figure(btc):
    ...
    ax.text(-0.35, 0.512, "0.5 = no skill (coin flip)",
            ha="left", fontsize=9.5, color=INK_SECONDARY)
    for xi, v in zip(x, aucs):
        ax.text(xi, v + 0.01, f"{v:.3f}", ha="center", fontsize=10, color=INK)
    ax.set_xticks(x)
    ax.set_xticklabels([f"{h} min" for h in horizons])
    ax.set_xlabel("prediction horizon (max_bars)")
...
def make_permutation_test_figure(btc, n_perms=60):
    ...
    ax.grid(axis="y", color=GRID, linewidth=0.8, zorder=0)
    ax.set_axisbelow(True)
    ax.legend(frameon=False, loc="upper left", fontsize=9.5, labelcolor=INK_SECONDARY)
    fig.tight_layout()
