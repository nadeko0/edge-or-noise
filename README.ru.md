...
Для точности, раз это легко округлить: именно эта строгая работа с
walk-forward валидацией — примерно **7 месяцев** (с декабря 2025 года).
До этого, с мая 2025-го, были пробные запуски на спотовой торговле с
базовыми индикаторами через TA-Lib — исследовательские, не на уровне
строгости, показанной в этом репозитории. Предпочту сказать это прямо,
чем создать впечатление, что такой уровень строгости существовал целый
год, хотя это не так.

Часть моей более ранней работы на свечных (не тиковых) данных тоже
давала бэктесты, которые выглядели впечатляюще — profit factor выше 2,
почти без просадки — но без permutation-теста и честной проверки вне
выборки за этим. Я больше не доверяю такому результату сам по себе —
именно поэтому этот репозиторий устроен так, как устроен: каждая
цифра здесь обязана пройти одни и те же три проверки (переживает ли
она null-распределение на перемешанных метках, держится ли на данных,
которых не видела, переживает ли небольшое изменение параметров) —
только после этого она что-то значит.
...
**Почему не все 24:** независимый архив 2024 года — самая строгая
проверка в этом репозитории — покрывает только BTC/ETH/SOL, так что
расширение батареи из ~80 гипотез на остальные 21 символ лишило бы её
единственной проверки, которая реально отличает настоящий паттерн от
ложного срабатывания. ETH/SOL выбраны, чтобы проверить, переносится ли
находка по BTC на другие ликвидные, устоявшиеся монеты, а не для скана
всего списка символов в поисках отдельного эджа; прогон той же батареи
по каждому символу также линейно растёт по времени вычислений (одна
только реконструкция L2-стакана занимает ~15 минут на символ). Скан
оставшихся альткоинов — которые тоньше и шумнее и потребовали бы
отдельной осторожности — это естественное продолжение, а не то, что
этот репозиторий заявляет как исключённое.
...
## Один реальный trade-off, не постановочный

Этот проект делался с ИИ-ассистентом в разработке. Я это не скрываю —
в 2026 году не так важно, был ли код сделан с помощью ИИ; важно,
может ли человек за ним объяснить и защитить каждое решение в нём. Вот
реальный обмен репликами из этого процесса — оставляю как есть,
потому что это лучше иллюстрирует суть, чем что-либо, написанное
задним числом:

**Вопрос: replay L2-стакана занимает ~15 минут на 90 дней BTC — это
ограничение железа или софта?**

В основном софта. Загрузчик дня в `orderbook_l2.py` парсит сырой JSON
построчно в обычный Python `dict`-based order book — его нельзя
векторизовать так, как остальной пайплайн (numba-JIT), потому что
каждая строка зависит от накопленного состояния предыдущих. Реальное
исправление простое, и я точно знаю, какое: 90 дней — это независимые
файлы без общего состояния между собой, то есть классический случай
для `multiprocessing.Pool` — распараллеливание по дням примерно вдвое
сократило бы время на машине с 2 ядрами, почти бесплатно.

Я этого не сделал. Как только результат вернулся чистым — глубина
стакана не двигает AUC (0.597 → 0.596 → 0.594, шум) — ускорять код,
давший нулевой результат, перестало быть оправданным по времени. Это
реальный trade-off ("работает и достаточно для вывода" против "работает
быстро"), а не незамеченная проблема — и это ровно тот тип решения,
который не виден в диффе, только когда тебя спрашивают напрямую.
